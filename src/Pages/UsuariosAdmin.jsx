...
function formatarTipo(tipo) {
  return tipo === "admin" ? "Administrador" : "Funcionario";
}
...
export default function UsuariosAdmin() {
  ...
  const abaAtiva = new URLSearchParams(location.search).get("aba") === "origens" ? "origens" : "perfis";
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSucesso(`${formatarTipo(form.tipo)} criado com sucesso.`);
  ...
  const handleAlternarStatus = async (usuarioAlvo) => {
    ...
    const confirmar = window.confirm(`Deseja ${acao} o usuario ${usuarioAlvo.nome || usuarioAlvo.email}?`);
    ...
    try {
      await atualizarStatusUsuario(usuarioAlvo.uid, proximoStatus);
      setSucesso(`Usuario ${proximoStatus ? "reativado" : "desativado"} com sucesso.`);
      await carregarUsuarios();
    } catch (error) {
    ...
    <main className="page-shell app-shell">
      <header className="page-header">
        <div>
          ...
          <h1>Opcoes</h1>
          <p>Configure perfis de acesso e as origens usadas no cadastro de clientes.</p>
        </div>
          ...
          <section className="panel usuarios-lista">
            <div className="section-heading">
              <div>
                ...
                <p className="muted-text">Desativar bloqueia o acesso sem apagar o historico desse perfil.</p>
              </div>
            </div>
                    ...
                    {usuarios.map((usuario) => {
                      ...
                      return (
                        <tr key={usuario.uid}>
                          ...
                          <td data-label="Tipo">{formatarTipo(usuario.tipo)}</td>
